...
function check_uml_links($result, $templateUmlLinksList, $testUmlLinksList, $templateUmlClassesList){
    ...
    foreach($templateUmlLinksList as $template){
        if ((property_exists($template, "from")) &&
            ...
            if (sizeof($found) > 1){
                $result[] = 'Existen enlaces redundantes entre las clases "'.$template->from.'" y "'.$template->to.'"';
            }elseif (sizeof($found) <= 0){
                $result[] = 'Falta un enlace entre las clases "'.$template->from.'" y "'.$template->to.'"';
            }else{
                if ($template->relationType != reset($found)->relationType){
                    $result[] = 'El enlace entre las clases "'.$template->from.'" y "'.$template->to.'" debe ser "'.$template->relationType.'"';
                }
            }
        }
    }
