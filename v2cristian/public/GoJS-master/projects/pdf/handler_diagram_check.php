...
try{
    ...
    $template = json_decode($diagram[3], false, 512, JSON_UNESCAPED_UNICODE);    // En la posicion 3 esta el JSON. Lo convierte a objeto.
    ...
    $conection = mysqli_connect('localhost', 'root', '', 'uml');
    $result = mysqli_query($conection, "insert into model_diagram_test values (NULL, '".$data->course_id."', '".$data->nactivity_id."', '".$user_name."', '".json_encode($data->diagramModel)."')");
    $conection->close();
